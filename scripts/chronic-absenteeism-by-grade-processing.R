...
for (i in 1:length(chronic_absent_dist_noTrend)) {
  ...
  colnames(current_file) = current_file[which( current_file$V3=='Grade'), ]
...
chronic_absent_dist$Grade <- gsub("   ", " ", chronic_absent_dist$Grade)
...
backfill_years <- expand.grid(
  ...
  `Grade` = unique(chronic_absent_dist$`Grade`),
  `Year` = years 
)
...
complete_chronic_absent <- merge(chronic_absent_fips, backfill_years, by = c("FixedDistrict", "Grade", "Year"), all=T)

complete_chronic_absent <- complete_chronic_absent[order(complete_chronic_absent$FixedDistrict, complete_chronic_absent$Year, complete_chronic_absent$`Grade`),]
...
write.table(
  ...
  file.path(path_to_top_level, "data", "chronic_absenteeism_by_grade_2012-2020.csv"),
  sep = ",",
  row.names = F
)
